  ...
  const LOCAL_USER = {
    id: 'local-access',
    nome: 'Setor Parametrização',
    email: 'parametrizacao@local',
    papel: 'administrador',
    ativo: true
  };
  const CACHE_KEY = 'radar-bootstrap-cache-empty-v1';
  const CACHE_TTL = 1000 * 60 * 60 * 24 * 365 * 20;
  ...
  const VIEW_META = {
    ...
    list: { title: 'Tarefas', action: '+ Nova Tarefa', sectionActions: true },
    subtasks: { title: 'Dashboard de Subtarefas', action: '+ Nova Tarefa', sectionActions: false },
    otherSectors: { title: 'Tarefas de Outros Setores', action: '+ Nova Tarefa', sectionActions: false },
    collaborators: { title: 'Colaboradores', action: '+ Novo Colaborador', sectionActions: false },
    departments: { title: 'Setores', action: '+ Novo Setor', sectionActions: false },
    access: { title: 'Acessos', action: '+ Novo Usuário', sectionActions: false },
    sla: { title: 'SLA', action: '+ Novo Setor', sectionActions: false },
  ...
  const getTaskSubtasks = (task) => parseJsonSafe(task?.subtarefas)
    .filter(sub => String(sub?.texto || '').trim() || sub?.concluido)
    .map((sub, index) => ({
      id: sub.id || index + 1,
      index,
      texto: String(sub.texto || '').trim() || `Subtarefa ${index + 1}`,
      concluido: !!sub.concluido,
      taskId: task?.id,
      taskTitle: task?.objetivo || '',
      taskStatus: task?.status || 'Não iniciado',
      taskDueDate: task?.prazo_conclusao || '',
      taskUrgency: getUrgency(task || {}),
      equipe: task?.equipe || '',
      responsavel: task?.responsavel || ''
    }));

  const getAllSubtasks = (tasks = App.tasks) => tasks.flatMap(task => getTaskSubtasks(task));
  ...
  const ensureUniqueTaskIds = (tasks = []) => {
    const seen = new Set();
    let changed = false;
    const normalized = tasks.map(task => {
      const currentId = String(task?.id || '').trim();
      if (!currentId || seen.has(currentId)) {
        changed = true;
        const nextId = makeLocalId('task');
        seen.add(nextId);
  ...
  const isParametrizacaoTask = (task) => normalizeText(task?.equipe).includes('parametrizacao');
  const getOtherSectorTasks = () => App.tasks.filter(task => task.equipe && !isParametrizacaoTask(task));
  ...
  const enableLocalAccess = () => {
    App.sessionToken = App.sessionToken || 'local-access';
    App.currentUser = App.currentUser
      ? { ...App.currentUser, papel: normalizeRole(App.currentUser.papel) }
      : { ...LOCAL_USER };
  };
  ...
  const localDemoAction = (body) => {
    switch (body.action) {
      case 'create': {
        return { ...body.payload, id: makeLocalId('task'), data_criacao: new Date().toISOString() };
      }
      case 'update': {
        return { ...body.payload };
      }
  ...
  const persistCurrentState = () => {
    const taskIdResult = ensureUniqueTaskIds(App.tasks);
    if (taskIdResult.changed) App.tasks = taskIdResult.tasks;
  ...
  const buildLocalBootstrap = () => {
    const cached = loadBootstrapCache();
    if (cached) return { ...cached, currentUser: App.currentUser || LOCAL_USER };

    const tasks = App.tasks.map(task => ({ ...task }));
    const departments = App.departments.map(department => ({ ...department }));
    const collaborators = App.collaborators.map(collaborator => ({ ...collaborator }));

    return {
      tasks,
      departments,
      collaborators,
      users: App.users.length ? App.users : [{ ...LOCAL_USER }],
      currentUser: App.currentUser || LOCAL_USER
    };
  };
  ...
  const hydrateAppData = (data) => {
    ...
    const taskIdResult = ensureUniqueTaskIds(Array.isArray(data.tasks) ? data.tasks : []);
    App.tasks = taskIdResult.tasks;
    ...
    if (App.sessionToken === 'local-access') {
      App.currentUser = { ...LOCAL_USER };
    } else if (data.currentUser) {
      App.currentUser = { ...data.currentUser, papel: normalizeRole(data.currentUser.papel) };
    }
    ...
    applyAccessControls();
    updateUserPill();
    if (taskIdResult.changed) persistCurrentState();
  };
  ...
  const updateCounts = () => {
    ...
    document.getElementById('cnt-subtasks').textContent = getAllSubtasks().length;
    document.getElementById('cnt-other-sectors').textContent = getOtherSectorTasks().length;
  ...
  const toggleDepartmentFilters = () => {
    ...
    if (!el || !btn) return;
    const isHidden = el.classList.toggle('filter-panel-collapsed');
    btn.textContent = isHidden ? 'Mostrar setores' : 'Ocultar setores';
    btn.classList.toggle('is-open', !isHidden);
  };
  ...
  const switchView = (v) => {
    ...
    App.currentView = v;
    ['kanban','list','subtasks','otherSectors','collaborators','departments','access','sla','indicators'].forEach(view => {
  ...
  const renderStats = () => {
    ...
    const subtasks = getAllSubtasks(tasks);
    const subtasksDone = subtasks.filter(sub => sub.concluido).length;
    ...
    statsEl.innerHTML = `
      ...
      <div class="stat-card" data-type="subtasks" onclick="switchView('subtasks')">
        <div class="stat-icon" style="background:var(--blue-dim);font-size:18px">☑️</div>
        <div class="stat-value" style="color:var(--blue)">${subtasks.length}</div>
        <div class="stat-label">Subtarefas</div>
        <div class="stat-trend">${subtasksDone}/${subtasks.length || 0} concluídas • ${subtasks.length ? Math.round((subtasksDone / subtasks.length) * 100) : 0}%</div>
      </div>
  ...
  const renderBoard = () => {
    const viewMap = {
      kanban: renderKanban, list: renderList, subtasks: renderSubtasksDashboard, otherSectors: renderOtherSectors, collaborators: renderCollaborators,
  ...
  const renderList = () => {
    ...
    container.innerHTML = `<div class="list-view"><table class="list-table">
      <thead><tr>
        <th>Tarefa</th><th>Setor</th><th>Responsável</th>
  ...
  const renderSubtasksDashboard = () => {
    ...
    const visibleTotal = visibleSubtasks.length;
    const done = allSubtasks.filter(sub => sub.concluido).length;
    const pending = total - done;
    const overdueOpen = allSubtasks.filter(sub => !sub.concluido && sub.taskUrgency === 'atrasado').length;
    const byParent = App.tasks
      .map(task => {
        const subs = getTaskSubtasks(task);
        const completed = subs.filter(sub => sub.concluido).length;
        return {
          id: task.id,
          title: task.objetivo || 'Tarefa sem titulo',
          total: subs.length,
          done: completed,
          pending: subs.length - completed,
          pct: pct(completed, subs.length)
        };
      })
    ...
    const dashboard = `
      <div class="subtask-dashboard">
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Subtarefas</div>
          <div class="subtask-dash-value" style="color:var(--accent)">${total}</div>
          <div class="subtask-dash-note">${visibleTotal !== total ? `${visibleTotal} visíveis pela busca` : 'Total cadastrado'}</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Concluídas</div>
          <div class="subtask-dash-value" style="color:var(--green)">${done}</div>
          <div class="subtask-dash-note">${pct(done, total)}% de conclusão</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Pendentes</div>
          <div class="subtask-dash-value" style="color:var(--yellow)">${pending}</div>
          <div class="subtask-dash-note">Ainda abertas</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Em tarefas atrasadas</div>
          <div class="subtask-dash-value" style="color:var(--red)">${overdueOpen}</div>
          <div class="subtask-dash-note">Pendentes com tarefa vencida</div>
        </div>
      </div>
      <div class="panel-grid subtask-report-grid">
        <div class="panel-card">
          <div class="panel-card-title">Progresso geral das subtarefas</div>
          ${total ? `
            <div class="report-bars">
              <div class="report-bar-row"><div class="report-bar-label"><span>Concluídas</span><strong>${done}</strong></div><div class="report-bar-track"><div class="report-bar-fill green" style="width:${pct(done, total)}%"></div></div></div>
              <div class="report-bar-row"><div class="report-bar-label"><span>Pendentes</span><strong>${pending}</strong></div><div class="report-bar-track"><div class="report-bar-fill yellow" style="width:${pct(pending, total)}%"></div></div></div>
              <div class="report-bar-row"><div class="report-bar-label"><span>Pendentes atrasadas</span><strong>${overdueOpen}</strong></div><div class="report-bar-track"><div class="report-bar-fill red" style="width:${pct(overdueOpen, total)}%"></div></div></div>
            </div>
          ` : '<div class="empty-col">Crie subtarefas dentro das tarefas para montar a dashboard</div>'}
        </div>
        <div class="panel-card">
          <div class="panel-card-title">Tarefas com mais pendências</div>
          ${byParent.length ? byParent.map(item => `
            <div class="metric-row subtask-parent-row" data-task-id="${escAttr(item.id)}">
              <div style="flex:1;min-width:0">
                <div class="metric-name">${escHtml(item.title)}</div>
                <div class="panel-card-subtitle">${item.done} concluídas • ${item.pending} pendentes • ${item.total} no total</div>
                <div class="progress-track" style="margin-top:10px"><div class="progress-fill" style="width:${item.pct}%"></div></div>
              </div>
              <div class="metric-value" style="color:${getStatusTone(item.pct)}">${item.pct}%</div>
            </div>
          `).join('') : '<div class="empty-col">Nenhuma tarefa com subtarefas</div>'}
        </div>
      </div>
    `;

    const list = visibleSubtasks.length ? `
      <div class="subtask-list-panel">
        <div class="panel-card-header">
          <div>
            <div class="panel-card-title">Lista separada de subtarefas</div>
            <div class="panel-card-subtitle">Clique em uma linha para abrir a tarefa principal</div>
          </div>
        </div>
        <div class="list-view">
          <table class="list-table">
            <thead><tr>
              <th>Subtarefa</th><th>Tarefa principal</th><th>Responsável</th><th>Prazo</th><th>Status</th><th>Ação</th>
            </tr></thead>
            <tbody>${visibleSubtasks.map(sub => `
              <tr class="subtask-row" data-task-id="${escAttr(sub.taskId)}" data-subtask-index="${sub.index}">
                <td class="cell-title">
                  <div class="subtask-list-title">
                    <span class="subtask-list-check${sub.concluido ? ' done' : ''}">${sub.concluido ? '✓' : ''}</span>
                    <div class="cell-title-text">${escHtml(sub.texto)}</div>
                  </div>
                </td>
                <td><div class="cell-title-text">${escHtml(sub.taskTitle || '—')}</div><div class="panel-card-subtitle">${escHtml(sub.equipe || 'Sem setor')}</div></td>
                <td style="color:var(--text-secondary);font-size:0.78rem">${escHtml(sub.responsavel || '—')}</td>
                <td style="color:var(--text-muted);font-size:0.78rem">${formatTaskDate(sub.taskDueDate, '—')}</td>
                <td><span class="tag ${sub.concluido ? 'tag-status-finalizado' : 'tag-status-em-andamento'}">${sub.concluido ? 'Concluída' : 'Pendente'}</span></td>
                <td>
                  ${sub.concluido
                    ? '<button class="btn-tiny" type="button" disabled>Concluída</button>'
                    : `<button class="btn-tiny btn-finish-subtask" type="button" data-task-id="${escAttr(sub.taskId)}" data-subtask-index="${sub.index}">Finalizar</button>`}
                </td>
              </tr>
            `).join('')}</tbody>
          </table>
    ...
    ` : '<div class="list-view" style="padding:40px;text-align:center;color:var(--text-muted)">Nenhuma subtarefa encontrada</div>';

    container.innerHTML = `${dashboard}${list}`;
    container.querySelectorAll('.btn-finish-subtask').forEach(btn => {
      btn.addEventListener('click', event => {
        event.stopPropagation();
        finishSubtaskFromDashboard(btn.dataset.taskId, Number(btn.dataset.subtaskIndex));
      });
    });
    container.querySelectorAll('.subtask-row, .subtask-parent-row').forEach(row => {
      row.addEventListener('click', () => openTaskDrawer(row.dataset.taskId));
    });
  };

  const finishSubtaskFromDashboard = (taskId, subtaskIndex) => {
    const taskIndex = App.tasks.findIndex(task => String(task.id) === String(taskId));
    if (taskIndex === -1 || Number.isNaN(subtaskIndex)) return;
    const subtasks = parseJsonSafe(App.tasks[taskIndex].subtarefas);
    if (!subtasks[subtaskIndex]) return;
    subtasks[subtaskIndex] = { ...subtasks[subtaskIndex], concluido: true };
    App.tasks[taskIndex] = {
      ...App.tasks[taskIndex],
      subtarefas: JSON.stringify(subtasks)
    };
    if (App.currentTask && String(App.currentTask.id) === String(taskId)) {
      App.currentTask = { ...App.currentTask, subtarefas: App.tasks[taskIndex].subtarefas };
    }
    persistCurrentState();
    syncAppChrome();
    toast('Subtarefa finalizada.', 'success', 1600);
  };

  const renderOtherSectors = () => {
    const container = document.getElementById('board-container');
    const tasks = filterBySearch(getOtherSectorTasks(), ['objetivo', 'equipe', 'responsavel', 'status', 'observacoes']);
    const finished = tasks.filter(task => isFinishedStatus(task.status)).length;
    const overdue = tasks.filter(task => getUrgency(task) === 'atrasado').length;
    const sectors = [...new Set(tasks.map(task => task.equipe).filter(Boolean))].length;

    const summary = `
      <div class="subtask-dashboard">
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Outros setores</div>
          <div class="subtask-dash-value" style="color:var(--accent)">${tasks.length}</div>
          <div class="subtask-dash-note">Tarefas fora da Parametrização</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Setores</div>
          <div class="subtask-dash-value" style="color:var(--blue)">${sectors}</div>
          <div class="subtask-dash-note">Setores com tarefas</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Finalizadas</div>
          <div class="subtask-dash-value" style="color:var(--green)">${finished}</div>
          <div class="subtask-dash-note">${pct(finished, tasks.length)}% do recorte</div>
        </div>
        <div class="subtask-dash-card">
          <div class="subtask-dash-label">Atrasadas</div>
          <div class="subtask-dash-value" style="color:var(--red)">${overdue}</div>
          <div class="subtask-dash-note">Precisam de atenção</div>
        </div>
      </div>
    `;

    if (!tasks.length) {
      container.innerHTML = `${summary}<div class="list-view" style="padding:40px;text-align:center;color:var(--text-muted)">Nenhuma tarefa de outro setor encontrada</div>`;
      return;
    }

    container.innerHTML = `${summary}<div class="subtask-list-panel">
      <div class="panel-card-header">
        <div>
          <div class="panel-card-title">Tarefas dos outros setores</div>
          <div class="panel-card-subtitle">Tarefas com setor diferente de Parametrização</div>
        </div>
      </div>
      <div class="list-view"><table class="list-table">
        <thead><tr>
          <th>Tarefa</th><th>Setor</th><th>Responsável</th><th>Prazo</th><th>Status</th><th>Urgência</th>
        </tr></thead>
        <tbody>${tasks.map(task => {
          const urgency = getUrgency(task);
          const label = isFinishedStatus(task.status) ? getFinishedStatusLabel(task) : urgencyLabel(urgency);
          const statusClass = 'tag-status-' + normalizeTaskStatus(task.status || 'Não iniciado').toLowerCase().replace(/ /g,'-').replace('ã','a').replace('é','e').replace('ó','o').replace('í','i');
          return `<tr class="task-row" data-id="${escAttr(task.id)}">
            <td class="cell-title"><div class="cell-title-text">${escHtml(task.objetivo)}</div></td>
            <td><span class="tag tag-equipe">${escHtml(task.equipe || '—')}</span></td>
            <td style="color:var(--text-secondary);font-size:0.78rem">${escHtml(task.responsavel || '—')}</td>
            <td style="color:var(--text-muted);font-size:0.78rem">${formatTaskDate(task.prazo_conclusao, '—')}</td>
            <td><span class="tag ${statusClass}">${escHtml(task.status || 'Não iniciado')}</span></td>
            <td><span style="font-size:0.78rem">${label}</span></td>
          </tr>`;
        }).join('')}</tbody>
      </table></div>
    </div>`;
  ...
  const renderCollaborators = () => {
    ...
    const hero = renderExecutiveHero(
      ...
      [
        ...
        { value: [...new Set(items.map(c => c.departamento).filter(Boolean))].length, label: 'Setores cobertos', color: 'var(--blue)' }
      ],
      [
    ...
    container.innerHTML = `${hero}<div class="management-grid">${items.map(c => {
      ...
      <div class="management-card is-clickable" onclick="focusCollaborator('${escAttr(c.nome)}','${escAttr(c.departamento || '')}')">
        ...
        <div class="management-meta">
          <span class="meta-pill">${escHtml(c.departamento || 'Sem setor')}</span>
  ...
  const renderDepartments = () => {
    ...
    if (!items.length) {
      container.innerHTML = `<div class="list-view" style="padding:40px;text-align:center;color:var(--text-muted)">Nenhum setor cadastrado</div>`;
      return;
    }
    const departmentTaskCount = items.reduce((acc, dep) => acc + App.tasks.filter(t => t.equipe === dep.nome).length, 0);
    const hero = renderExecutiveHero(
      'Arquitetura de setores com leitura operacional.',
      'Visual de gestão para enxergar volume, saúde de SLA, concentração de tarefas e impacto de cada setor sem perder o contexto de execução.',
      [
        { value: items.length, label: 'Setores mapeados', color: 'var(--accent)' },
        ...
        <div class="dept-card">
          ...
          <div style="margin-top:12px">
            <div class="metric-row" style="padding-top:0">
              <span class="metric-name">Conclusão do setor</span>
  ...
  const renderSla = () => {
    ...
    const hero = renderExecutiveHero(
      ...
      'Um cockpit para identificar gargalos por setor, risco de vencimento e tendência de cumprimento antes de a operação travar.',
      [
      ...
      [
        { label: 'Setor crítico', value: rows[0]?.dep?.nome || '—', color: 'var(--red)' },
    ...
    container.innerHTML = `
      ...
      <div class="panel-grid">
        <div class="panel-card">
          <div class="panel-card-header">
            <div>
              <div class="panel-card-title">Mapa de SLA por setor</div>
          ...
          `).join('') : '<div class="empty-col">Cadastre setores para acompanhar SLA</div>'}
        </div>
        <div class="panel-stack">
          <div class="panel-card">
            <div class="panel-card-title">Leituras rápidas</div>
            <div class="metric-row"><span class="metric-name">Setores monitorados</span><span class="metric-value">${App.departments.length}</span></div>
          ...
          <div class="panel-card">
            ...
            <div class="table-note">Leitura baseada no prazo de conclusão da tarefa e na estrutura atual de setores.</div>
          </div>
        </div>
      </div>
    `;
  ...
  const renderIndicators = () => {
    ...
    const scopedTasks = App.indicatorFocus
      ...
      : App.tasks.slice();
    const scopedSubtasks = getAllSubtasks(scopedTasks);
    const completedTasks = scopedTasks.filter(t => isFinishedStatus(t.status)).length;
    const openTasks = scopedTasks.length - completedTasks;
    const overdueTasks = scopedTasks.filter(t => getUrgency(t) === 'atrasado').length;
    const doneSubtasks = scopedSubtasks.filter(sub => sub.concluido).length;
    const openSubtasks = scopedSubtasks.length - doneSubtasks;
    ...
    const topResponsaveis = Object.entries(responsaveisMap)
      ...
      .slice(0, 6);
    const completedByCollaborator = Object.entries(responsaveisMap)
      .map(([nome, m]) => ({ nome, total: m.total, concluidas: m.concluidas, pct: pct(m.concluidas, m.total) }))
      .filter(item => item.total)
      .sort((a, b) => b.concluidas - a.concluidas || b.pct - a.pct || a.nome.localeCompare(b.nome, 'pt-BR'))
      .slice(0, 6);

    const equipesMap = scopedTasks.reduce((acc, t) => {
      if (!t.equipe) return acc;
      if (!acc[t.equipe]) acc[t.equipe] = { total: 0, atrasadas: 0, naoIniciado: 0, andamento: 0, finalizado: 0 };
      acc[t.equipe].total++;
      if (getUrgency(t) === 'atrasado') acc[t.equipe].atrasadas++;
      if (normalizeTaskStatus(t.status) === 'Não iniciado') acc[t.equipe].naoIniciado++;
    ...
    const subtaskByTask = scopedTasks
      .map(task => {
        const subs = getTaskSubtasks(task);
        const done = subs.filter(sub => sub.concluido).length;
        return {
          id: task.id,
          title: task.objetivo || 'Tarefa sem titulo',
          total: subs.length,
          done,
          open: subs.length - done,
          pct: pct(done, subs.length)
        };
      })
      .filter(item => item.total)
      .sort((a, b) => b.total - a.total)
    ...
    const hero = renderExecutiveHero(
      App.indicatorFocus ? `Relatórios de ${escHtml(App.indicatorFocus)}.` : 'Relatórios de tarefas e subtarefas.',
      App.indicatorFocus ? 'Foco individual aplicado apenas dentro desta aba. Ao sair dela, o filtro é limpo automaticamente.' : 'Indicadores separados para acompanhar a tarefa principal e a execução detalhada das subtarefas.',
      [
        { value: scopedTasks.length, label: 'Tarefas principais', color: 'var(--accent)' },
        { value: scopedSubtasks.length, label: 'Subtarefas cadastradas', color: 'var(--blue)' },
        { value: `${pct(doneSubtasks, scopedSubtasks.length)}%`, label: 'Conclusão de subtarefas', color: 'var(--green)' }
      ],
      [
        { label: 'Destaque individual', value: bestKpi?.nome || '—', color: 'var(--green)' },
        { label: 'Setor mais carregado', value: topEquipes[0]?.nome || '—' },
        { label: 'Tarefas atrasadas', value: overdueTasks, color: 'var(--red)' }
      ]
    );
    ...
    container.innerHTML = `
      ...
      <div class="panel-grid">
        <div class="panel-card">
          <div class="panel-card-header">
            <div>
              <div class="panel-card-title">Relatório de tarefas principais</div>
              <div class="panel-card-subtitle">Status geral das tarefas cadastradas</div>
            </div>
          </div>
          ${scopedTasks.length ? `
            <div class="report-bars">
        ...
        <div class="panel-stack">
          <div class="panel-card">
            <div class="panel-card-title">Tarefas concluídas por colaborador</div>
            <div class="panel-card-subtitle">Ranking de finalização individual</div>
            ${completedByCollaborator.length ? completedByCollaborator.map(item => `
              <div class="metric-row">
                <div style="flex:1;min-width:0">
                  <div class="metric-name">${escHtml(item.nome)}</div>
                  <div class="panel-card-subtitle">${item.concluidas} concluídas • ${item.total} no total</div>
                  <div class="progress-track" style="margin-top:10px"><div class="progress-fill" style="width:${item.pct}%"></div></div>
                </div>
                <div class="metric-value" style="color:${getStatusTone(item.pct)}">${item.pct}%</div>
              </div>
            `).join('') : '<div class="empty-col">Sem tarefas concluídas por colaborador</div>'}
          </div>
          <div class="panel-card">
          ...
          <div class="panel-card">
            <div class="panel-card-title">Subtarefas por tarefa</div>
            ${subtaskByTask.length ? subtaskByTask.map(item => `
              <div class="metric-row">
                <div style="flex:1;min-width:0">
                  <div class="metric-name">${escHtml(item.title)}</div>
                  <div class="panel-card-subtitle">${item.done} concluídas • ${item.open} pendentes • ${item.total} no total</div>
                  <div class="progress-track" style="margin-top:10px"><div class="progress-fill" style="width:${item.pct}%"></div></div>
                </div>
                <div class="metric-value" style="color:${getStatusTone(item.pct)}">${item.pct}%</div>
              </div>
  ...
  const openDepartmentDrawer = (id) => {
    if (!canManageStructure()) {
      toast('Seu perfil não pode alterar setores.', 'warning');
      return;
    }
    ...
    document.getElementById('drawer-title').textContent = App.currentDepartment ? 'Editar Setor' : 'Novo Setor';
    ...
    document.getElementById('drawer-body').innerHTML = `
      ...
      <div class="drawer-tab-panel${currentTab === 'overview' ? '' : ' view-hidden'}" data-tab="overview">
        ...
        <div class="form-group">
          ...
          <textarea class="form-textarea" id="f-obs" placeholder="Notas da tarefa..." ${readOnlyTask ? 'readonly' : ''}>${escHtml(t.observacoes || '')}</textarea>
        </div>
        ${subs.length ? `
          <div class="subtasks-section">
            <div class="subtasks-header">
              <span class="subtasks-title">Subtarefas da tarefa principal</span>
              ${prog ? `<span class="subtask-progress-mini">${prog.done}/${prog.total} - ${prog.pct}%</span>` : ''}
            </div>
            <div class="subtask-list">
              ${subs.slice(0, 4).map((s, i) => `
                <div class="subtask-item${s.concluido ? ' done' : ''} read-only">
                  <div class="subtask-checkbox read-only">${s.concluido ? '✓' : ''}</div>
                  <div class="subtask-text">${escHtml(s.texto || `Subtarefa ${i + 1}`)}</div>
                </div>
              `).join('')}
            </div>
            ${subs.length > 4 ? `<div class="table-note">${subs.length - 4} subtarefa${subs.length - 4 > 1 ? 's' : ''} a mais na aba Subtarefas.</div>` : ''}
          </div>
        ` : ''}
      </div>
  ...
  const renderDepartmentDrawerForm = (dep) => {
    ...
    document.getElementById('drawer-body').innerHTML = `
      <div class="form-group">
        ...
        <input class="form-input" id="d-nome" type="text" value="${escHtml(d.nome || '')}" placeholder="Ex.: Parametrização" />
      </div>
      <div class="form-group">
  ...
  const updateSubtaskText = (i, val) => {
    syncCurrentSubtasksToTask({ persist: false });
  };
  ...
  const syncCurrentSubtasksToTask = ({ persist = true } = {}) => {
    if (!App.currentTask) return;
    const subtarefas = JSON.stringify(getCurrentSubtasks());
    App.currentTask.subtarefas = subtarefas;
    const idx = App.tasks.findIndex(task => String(task.id) === String(App.currentTask.id));
    if (idx !== -1) App.tasks[idx] = { ...App.tasks[idx], subtarefas };
    if (persist && idx !== -1) {
      persistCurrentState();
      if (isTaskView(App.currentView) || App.currentView === 'indicators') {
        syncAppChrome();
      }
    }
  };
  ...
  const saveTask = async () => {
    ...
    try {
      if (App.currentTask) {
        ...
        const tempId = makeLocalId('draft');
  ...
  const saveDepartment = async () => {
    if (!canManageStructure()) return toast('Seu perfil não pode alterar setores.', 'warning');
    ...
    if (!nome) return toast('Informe o nome do setor', 'warning');
    ...
    try {
      if (App.currentDepartment) {
        ...
        await updateUIAndSync('Setor atualizado!', 'updateDepartment', payload);
      } else {
        const tempId = 'dep-' + Date.now();
        App.departments.unshift({ ...payload, id: tempId, data_criacao: new Date().toISOString() });
        await updateUIAndSync('Setor criado!', 'createDepartment', payload, tempId, App.departments);
      }
    } catch (e) {
  ...
  const deleteCurrentDepartment = async () => {
    if (!canManageStructure()) return toast('Seu perfil não pode excluir setores.', 'warning');
    if (!App.currentDepartment) return;
    const confirmed = await requestConfirm({
      title: 'Excluir setor',
      message: `O setor "${App.currentDepartment.nome}" será removido permanentemente.`,
      confirmLabel: 'Excluir'
    });
    if (!confirmed) return;
    ...
    if (hasLinks) {
      toast('Remova ou mova tarefas e colaboradores antes de excluir este setor', 'warning', 5000);
      return;
    }
  ...
  (function init() {
    ...
    loadTasks({ useCache: false, silent: !!cached })
      .then(() => showAppShell())
      .catch(() => {
        hydrateAppData(buildLocalBootstrap());
        syncAppChrome();
        showAppShell();
      });
    renderIcons();
